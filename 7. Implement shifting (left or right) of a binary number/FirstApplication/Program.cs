...
namespace FirstApplication
{
    class Program
    {
        static void Main()
        {
            ...
            const int optionNumberSeven = 7;
            const int optionNumberEight = 8;
            ...
            else if (operationSelect == optionNumberSeven)
            {
                ApplyShiftLeftOperator(inputString, Convert.ToInt32(Console.ReadLine()));
            }
            else if (operationSelect == optionNumberEight)
            {
                ApplyShiftRightOperator(inputString, Convert.ToInt32(Console.ReadLine()));
            }
            else
            {
        ...
        static void ApplyShiftLeftOperator(string input, int secondInput)
        {
            char[] arr = input.ToCharArray(0, input.Length);

            if (secondInput < 1)
            {
                Console.WriteLine("The shifting position number should be a positive integer.");
                return;
            }
            ...
            char[] addedZeroes = new char[arr.Length + secondInput];
            for (int i = 0; i < addedZeroes.Length; i++)
            {
                addedZeroes[i] = '0';
            }

            Array.Copy(arr, 0, addedZeroes, 0, arr.Length);

            string result = new string(addedZeroes);
            Console.WriteLine(result);
        }

        static void ApplyShiftRightOperator(string input, int secondInput)
        {
            char[] arr = input.ToCharArray(0, input.Length);
            ...
            else if (arr.Length < secondInput)
            {
                Console.WriteLine('0');
                return;
            }

            char[] shiftedRightArray = new char[arr.Length - secondInput];
            for (int i = 0; i < shiftedRightArray.Length; i++)
            {
                shiftedRightArray[i] = input[i];
            }

            string result = new string(shiftedRightArray);
            Console.WriteLine(result);
        }
    }
}